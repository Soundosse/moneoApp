...
class _DashboardPageState extends State<DashboardPage> {
  ...
  // ============================================================
  // LOAD USER DATA - FAST VERSION
  //
  // IMPORTANT:
  //
  // We DO NOT wait for Firestore before displaying the name.
  //
  // Firebase Auth already contains displayName.
  // ============================================================
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          userPhotoUrl =
          authPhoto.isEmpty ? null : authPhoto;
      ...
      debugPrint(
        'Dashboard FAST - Auth displayName: $authName',
      );

      // --------------------------------------------------------
      // STEP 2
      // Read Firestore in background
      //
      // We do NOT block the UI.
      // --------------------------------------------------------

      _loadFirestoreUserData(user);
    } catch (e) {
      debugPrint(
        'Dashboard - erreur chargement utilisateur: $e',
      );
  ...
  // FIRESTORE USER DATA
  //
  // Runs AFTER the Auth information has already been displayed.
  // ============================================================

  Future<void> _loadFirestoreUserData(User user) async {
    try {
      ...
      if (!snapshot.exists) {
        debugPrint(
          'Dashboard - users/${user.uid} does not exist.',
        );
      ...
      // --------------------------------------------------------
      // Update only if Firestore actually has useful data.
      // --------------------------------------------------------

      if (!mounted) return;

      setState(() {
        // Firestore name has priority ONLY if it exists.
      ...
      debugPrint(
        'Dashboard FIRESTORE - displayName: $firestoreName',
      );
    } catch (e) {
      debugPrint(
        'Dashboard - Firestore user error: $e',
      );

      // --------------------------------------------------------
      // IMPORTANT:
      // Do nothing to the UI.
      //
      // Auth already provided the name.
      // --------------------------------------------------------
    }
  }
        ...
        children: [
          ...
          Container(
            ...
            BoxDecoration(
              ...
              color:
              cyan.withOpacity(0.06),

              border: Border.all(
                color:
                cyan.withOpacity(0.32),
              ),
            ),
